...
invariantsnod={mTsq,qsq,s12,s15,s23,s34,s45};
invariantsnodUpperCase={MTsq,Qsq,S12,S15,S23,S34,S45};

p4conv={p[5]->-(p[1]+p[2]+p[3]+p[4])};
...
diffOpAnsatz[expr_] := Module[{v1},
	  v1 = Sum[da[j, i]*p[j]*D[expr, p[i]], {i, 1, 5}, {j, 1, 4}];
	  v1 = v1 + Sum[ds[invariantsnodUpperCase[[i]]]*D[expr, invariantsnod[[i]]], {i, 1, Length[invariantsnod]}];
	  Return[v1];
	  ];


diffOpAnsatz2 =Sum[da[j, i]*p[j]*del[p[i]], {i, 1, 5}, {j, 1, 4}] + Sum[ds[invariantsnodUpperCase[[i]]]*del[invariantsnod[[i]]], {i, 1, Length[invariantsnod]}];
...
p4diffopConst=Table[Expand[p[i]diffOpAnsatz[p[1]+p[2]+p[3]+p[4]+p[5]]/.p4conv],{i,1,4}];
...
checkDifferentialOperator[tpoint_,diffop_] := Module[{diffopS,invariantsnod2,InvariantsConstraintsCheck,InvariantsMandsCheck,CheckMatrix,kinematicsT,mandelstamScalarProductsT},
	invariantsnod2={mTsq,qsq,s12,s15,s23,s34,s45};
	mandelstamScalarProductsT=Expand[mandelstamScalarProducts/.tpoint,Modulus->$P];
	InvariantsConstraintsCheck={(p[1])^2,(p[2])^2,(p[3])^2,(p[4])^2,(p[5])^2,(p[1]+p[2])^2,(p[1]+p[5])^2,(p[2]+p[3])^2,(p[3]+p[4])^2,(p[4]+p[5])^2};
	InvariantsMandsCheck={mTsq,qsq,mTsq,0,0,s12,s15,s23,s34,s34,s45};
   ...
   {{Transpose[ArrayReshape[Append[Range[1,11],InvariantsMandsCheck]//Flatten,{2,11}]], Transpose[ArrayReshape[Append[Range[1,11],InvariantsMandsCheck]//Flatten,{2,11}]]}, {Transpose[ArrayReshape[Append[Range[1,Length[invariantsnod2]],invariantsnod2]//Flatten,{2,Length[invariantsnod2]}]], Transpose[ArrayReshape[Append[Range[1,Length[invariantsnod2]],invariantsnod2]//Flatten,{2,Length[invariantsnod2]}]]}}]//Echo;
